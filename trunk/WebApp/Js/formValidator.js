...
$.formValidator = 
{
	...
	initConfig : function(controlOptions)
	{
		var settings = 
		{
			...
			errorfocus:true,
			wideword:true,
                        autoSubmit:true//liusylon
		};
		controlOptions = controlOptions || {};
		$.extend(settings, controlOptions);
		//如果是精简模式，发生错误的时候，第一个错误的控件就不获得焦点
		if(settings.tidymode){settings.errorfocus=false};
		// if(settings.formid!=""){$("#"+settings.formid).submit(function(){ var r=$.formValidator.pageIsValid("1");if(r){return settings.autoSubmit;} else {return false;}})};
		if(settings.formid!=""){$("#"+settings.formid).live("submit",function(e){ if(!settings.autoSubmit){e.preventDefault()}; return $.formValidator.pageIsValid("1");})};
	...
	regexValid : function(returnObj)
	{
		...
		else 
		{
			...
			setting.isvalid = (new RegExp(regexpress)).test($("#"+id).val());
		}
	},
