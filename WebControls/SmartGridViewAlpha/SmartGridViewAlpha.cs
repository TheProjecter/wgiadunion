...
namespace Wgi.Web.UI.Controls.RichGridViewAlpha
{
    ...
    /// </summary>
    [
    ToolboxData(@"<{0}:RichGridViewAlpha runat='server'></{0}:RichGridViewAlpha>"),
    System.Drawing.ToolboxBitmap(typeof(RichGridViewAlpha), "icon.bmp"),
    ToolboxItem(false),
    ParseChildren(true),
    PersistChildren(false)
    ]
    public class RichGridViewAlpha : GridView, IPostBackDataHandler
    {
        /// <summary>
        /// 构造函数
        /// </summary>
        public RichGridViewAlpha()
        {
            this.PreRender += new EventHandler(RichGridViewAlpha_PreRender);
        }

        /// <summary>
        /// RichGridViewAlpha_PreRender
        /// </summary>
        /// <param name="sender">sender</param>
        /// <param name="e">e</param>
        void RichGridViewAlpha_PreRender(object sender, EventArgs e)
        {
            ...
            {
                // 滚动条x位置
                Page.ClientScript.RegisterHiddenField("yy_RichGridViewAlpha_x", _yy_RichGridViewAlpha_x.ToString());
                // 滚动条y位置
                Page.ClientScript.RegisterHiddenField("yy_RichGridViewAlpha_y", _yy_RichGridViewAlpha_y.ToString());

                // 设置GridView的滚动条的位置
                Page.ClientScript.RegisterStartupScript(
                    this.GetType(),
                    "jsSetScroll", "<script type=\"text/javascript\">document.getElementById('yy_ScrollDiv').scrollLeft=" + _yy_RichGridViewAlpha_x + ";document.getElementById('yy_ScrollDiv').scrollTop=" + _yy_RichGridViewAlpha_y + ";</script>"
                    );
        ...
        protected override void Render(HtmlTextWriter writer)
        {
            ...
            if (!FixRowCol.TableWidth.IsEmpty || !FixRowCol.TableHeight.IsEmpty)
            {
                ...
                if (FixRowCol.EnableScrollState)
                {
                    writer.Write("onscroll=\"document.getElementById('yy_RichGridViewAlpha_x').value = this.scrollLeft; document.getElementById('yy_RichGridViewAlpha_y').value = this.scrollTop;\">");
                }
                else
                {
                    writer.Write(">");
        ...
        public int ScrollX
        {
            get { return this._yy_RichGridViewAlpha_x; }
        }
        ...
        public int ScrollY
        {
            get { return this._yy_RichGridViewAlpha_y; }
        }
        #endregion
        ...
        /// 如果固定行、列的话 滚动条的x位置
        /// </summary>
        private int _yy_RichGridViewAlpha_x;
        /// <summary>
        /// 如果固定行、列的话 滚动条的y位置
        /// </summary>
        private int _yy_RichGridViewAlpha_y;
        /// <summary>
        /// 是否触发RowDataBound事件
        /// </summary>
        ...
        bool IPostBackDataHandler.LoadPostData(string postDataKey, NameValueCollection postCollection)
        {
            ...
            _yy_RichGridViewAlpha_x = String.IsNullOrEmpty(postCollection["yy_RichGridViewAlpha_x"]) ? 0 : Convert.ToInt32(postCollection["yy_RichGridViewAlpha_x"]);
            _yy_RichGridViewAlpha_y = String.IsNullOrEmpty(postCollection["yy_RichGridViewAlpha_y"]) ? 0 : Convert.ToInt32(postCollection["yy_RichGridViewAlpha_y"]);
